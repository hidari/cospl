...
- trim する
- 改行・制御文字を除去する（単一行スロットに偽の条項や見出しを注入させない）
- `<` `>` を除去する（ダウンロードした Markdown が後段で HTML 描画された場合の休眠 XSS への保険）
- 長さ clamp: 撮影者名 50 文字・連絡先 100 文字
- 日付は `^\d{4}-\d{2}-\d{2}$` にマッチしなければプレースホルダにフォールバック
...
4. `sanitizeFields` の各エッジ: 改行除去・制御文字除去・`<>` 除去・長さ clamp・不正日付フォールバック・空 → プレースホルダ
